...
방문 일자와 주문한 메뉴 정보를 기반으로 총 주문 금액, 프로모션 혜택 결과, 프로모션 영수증,
그리고 이벤트 뱃지를 생성하여 이벤트 결과를 종합적으로 제공하는 서비스입니다.

## 📌 기능 요구 사항

---

### 1️⃣ 12월달 예상 “방문 날짜”를 사용자로부터 입력 받는다.
...
### 6️⃣ 할인 전 총주문 금액에 따라 증정 메뉴를 출력할 수 있다.
...
### 7️⃣ 방문하는 날짜에 받을 수 있는 혜택 내역을 출력할 수 있다.
...
- [x] 고객에 적용된 이벤트가 하나도 없다면 혜택 내역 "없음"으로 표시한다.
    ```text
    <혜택 내역>
    없음
    ```
- [x] 할인 & 이벤트 진행 방법은 아래와 같다.
    ...
    - 평일 할인 (일요일~목요일)
      - 평일 할인 기간에 디저트 메뉴는 1개당 2,023원 할인
    - 주말 할인 (금요일,토요일)
      - 주말 할인 기간에 메인 메뉴는 1개당 2,023원 할인
...
- [x] 총혜택 금액을 계산하여 출력한다.
- ‘할인 전 총주문 금액’ 10,000원 이상에 해당하지 않아 혜택 내역이 “없음”인 경우는 총혜택 금액도 없음으로 0원으로 출력한다.
    ```text
    <총혜택 금액>
    0원
    ```
- [x] 총혜택 금액 = 할인 금액의 합계 + 증정 메뉴의 가격
- [x] 할인 전 총주문 금액이 120,000원이 넘지 않아 증정 이벤트에 해당하지 않는 경우는 증정 메뉴의 가격을 0원으로 간주한다.
...
- [x] 할인 후 예상 결제 금액 = 할인 전 총주문 금액 - 할인 금액
    - ❗여기서, 할인 금액에는 증정 이벤트 메뉴의 가격은 포함하지 않는다❗
